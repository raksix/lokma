...
- [REQ-149](REQ-149-sessionlar-websocket-canli.md) — Sessions bound to the websocket: list + transcript requested and pushed over WS instead of the 4 s REST poll — in-progress (tick 4/5: store folds the pushed rows, sidebar polls only while socket-less, chat asks its transcript over the socket)
...
- [REQ-154](finished/REQ-154-tarayici-etkilesim-araclari.md) — The agent can only OPEN a URL: add real page interaction (read / scroll / click / type) via a server-side browser engine, since a cross-origin iframe cannot be scripted from the client — done (Playwright engine + 5 tools; live E2E 7/7 — the agent scrolled a real page and reported scrollY 805→1525)
