...
import { Info, Plug2 } from 'lucide-react';
 ...
 * the real W2-5 pane (live CRUD + connection test). Later W2 slices
 * (Models/Usage/Config/…) add tabs here; the W7 pane system may relocate
 * the whole panel without touching the panes themselves.
 */
export function InspectorPanel() {
  const [tab, setTab] = React.useState<'info' | 'providers'>('info');
  ...
  return (
    <div className="space-y-3">
      ...
      {tab === 'info' ? <InfoPanel /> : <ProvidersPane />}
    </div>
  );
}
