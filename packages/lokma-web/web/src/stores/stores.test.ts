...
// REQ-156 — the same persisted row can reach the cache more than once (the chat
...
assert(isSameTranscriptRow({ role: 'user', content: 'x', timestamp: 't' }, { role: 'user', content: 'x', timestamp: 't' }) === true,
  'identical transcript rows are recognised as the same line (REQ-156)');
assert(isSameTranscriptRow({ role: 'user', content: 'x', timestamp: 't' }, { role: 'user', content: 'x', timestamp: 't2' }) === false,
  'a different timestamp is a different row (REQ-156)');
assert(isSameTranscriptRow({ role: 'assistant', content: 'x', timestamp: 't' }, { role: 'user', content: 'x', timestamp: 't' }) === false,
  'a different role is a different row (REQ-156)');
assert(isSameTranscriptRow(null, { role: 'user' }) === false, 'a missing row never matches (REQ-156)');
...
assert(useSessionStore.getState().transcripts['sess_1'].length === 1, 'a duplicate push does not grow the transcript (REQ-156)');
...
assert(useSessionStore.getState().transcripts['sess_1'].length === 2, 'a genuinely new row still appends (REQ-156)');
