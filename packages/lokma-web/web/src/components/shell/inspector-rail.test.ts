...
check('info first, memory last', tabs[0] === 'info' && tabs[tabs.length - 1] === 'memory');
check(
  'covers every Inspector menu',
  ['info', 'providers', 'models', 'usage', 'settings', 'terminal', 'git', 'browser', 'agents', 'orchestration', 'vault', 'skills', 'archify', 'design', 'testing', 'bots', 'auth', 'setup', 'plugins', 'observability', 'cron', 'extras', 'memory'].every(
