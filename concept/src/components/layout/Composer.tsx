...
import { Paperclip, Mic, Search, Sparkles, X, LifeBuoy, ListTree, ChevronDown } from "lucide-react"
...
export function Composer({ placeholder = "Ask Lokma to plan, code, or explain — try “add 60/min rate limit per user”", onSend }: { placeholder?: string; onSend?: (text: string, files: File[], mode: string, model: string) => void }) {
  ...
  return (
    <div
      ...
      className="rounded-xl bg-white border border-line shadow-[0_1px_2px_rgba(38,38,36,0.06),0_4px_12px_rgba(38,38,36,0.04)] overflow-visible relative data-[drag=1]:ring-2 data-[drag=1]:ring-terracotta/30 data-[drag=1]:border-terracotta/30 transition">
      {/* Top chip row — image gibi Onboarding + Steer/Queue + Muse Spark */}
      <div className="flex items-center gap-1 px-2 py-1 border-b border-line/50 bg-[#FDFCFB] dark:bg-[#161618] flex-wrap rounded-t-xl">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <div className="relative">
            <Button variant="outline" size="sm" className="h-6 pl-1.5 pr-1 rounded-full bg-white border-line text-xs gap-1 max-w-[150px]" onClick={() => setOpen(!open)} title="Model seç">
              <span className="w-4 h-4 rounded-full bg-[#262624] text-white grid place-items-center text-[9px] shrink-0">◐</span>
              <span className="truncate">{MODELS.flatMap(g => g.items).find(m => m.id === model)?.label || "Muse Spark"}</span>
              <ChevronDown className={cn("w-3 h-3 shrink-0 text-zinc-400 transition-transform", open && "rotate-180")} />
            </Button>
            {open && (
              <>
                <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
                <div className="absolute bottom-[calc(100%+8px)] right-0 w-[340px] max-w-[92vw] z-50 rounded-xl bg-[#111113] border border-[#2A2A2E] shadow-2xl overflow-hidden flex flex-col max-h-[420px] animate-in fade-in slide-in-from-bottom-1">
                  <div className="p-2 border-b border-white/10">
                    <div className="relative">
                      <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-500" />
                      <input autoFocus placeholder="Search models" value={query} onChange={e => setQuery(e.target.value)} className="w-full h-8 pl-8 pr-3 rounded-lg bg-[#1E1E20] border border-white/10 text-[13px] text-white placeholder:text-zinc-500 focus:outline-none focus:border-white/20" />
                    </div>
                  </div>
                  <div className="flex-1 overflow-y-auto py-1">
                    {filtered.length === 0 && <div className="px-3 py-6 text-center text-xs text-zinc-500">Model bulunamadı</div>}
                    {filtered.map(g => (
                      <div key={g.group}>
                        <div className="px-2.5 pt-2 pb-1 text-[10px] font-semibold tracking-widest uppercase text-zinc-500">{g.group}</div>
                        {g.items.map(m => (
                          <Button key={m.id} variant="ghost" size="sm" onClick={() => { setModel(m.id); setOpen(false); setQuery(""); window.dispatchEvent(new CustomEvent("lokma-toast", { detail: `Model: ${m.label}` })) }} className={cn("w-[calc(100%-8px)] mx-1 justify-between text-[13px] text-white hover:bg-white/10 hover:text-white", model === m.id && "bg-white/10 border border-white/5")}>
      ...
      <div className="p-1.5">
        ...
        <div className="flex items-center gap-1.5 mt-2">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <Button onClick={handleSend} className="h-7 pl-3 pr-1.5 rounded-full gap-1 bg-[#C96442] hover:bg-[#B85736] text-white border-0">
              Send
      ...
      <div className="flex items-center gap-1 px-2 py-1 border-t border-line/50 bg-[#FDFCFB] dark:bg-[#161618] overflow-x-auto rounded-b-xl">
        {contexts.map(c => (
