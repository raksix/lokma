...
- **Status:** done (2026-09-11 acceptance E2E — all FAZ A/B/C/D shipped, probe 160/160) (FAZ A shipped 2026-09-11 — stop_reason discipline + max_turns transcript marker; FAZ B-engine shipped 2026-09-11 — `server/src/engines/claude-print.ts` translator + spawn path, 29-assert probe green; FAZ B-wiring shipped 2026-09-11 — `claude-code/*` dispatch in ws.ts pump + runClaudeEngineTurn persist/account, 41-assert probe green; FAZ C-bridge shipped 2026-09-11 — `resolveClaudePermissions` config-driven allow/deny (deny-wins, Bash(rm *) inviolable), 52-assert probe green; FAZ D-continuity shipped 2026-09-11 — meta claudeSessionId persist+resume, store probe +4; FAZ D-compact-window shipped 2026-09-11 — pre-turn auto-compact (hygiene/full) + [compact] marker + LOKMA_DISABLE_AUTO_COMPACT kill-switch, warn-only; FAZ C-ask-gate shipped 2026-09-11 — pre-spawn permission cards for ask-fated mutation surface via decideToolCall + shared gate path, 59-assert probe green; FAZ D-clear shipped 2026-09-11 — harness-side `/clear` drops the resume handle (fresh next run, no spawn, [cleared] marker + done), 66-assert probe green; FAZ D-compact shipped 2026-09-11 — explicit `/compact` compacts the Lokma-side transcript harness-side in default full mode (never spawns the binary, explicit counterpart to the pre-turn auto-compact window) + shared `claudeCompactMarker` for identical marker text on both paths, 74-assert probe green; FAZ D-context shipped 2026-09-11 — explicit `/context` reports headless-run context state harness-side (message/char counts vs hygiene/summary budgets + resume handle + per-run budget + last compact; fresh sessions report fresh state, never spawns the binary), 102-assert probe green; FAZ D-compact-focus shipped 2026-09-11 — `/compact <focus>` parses harness-side (500-char cap, honest [truncated]) + focus line rides the marker row into the next turn, never spawns the binary, 115-assert probe green; FAZ D-context-categories shipped 2026-09-11 — `/context` gains per-role category breakdown (`countClaudeCategories` + `formatClaudeCategoryLine`, optional input field keeps old callers/probes green, ws.ts reads the transcript and degrades to no line on read failure), 122-assert probe green; FAZ D-reinject-quota shipped 2026-09-11 — `[reinject: ...]` skill-guidance row after every successful compact (explicit + pre-turn paths, max 3 per last-20 transcript window, no-op/failed compacts never reinject), 132-assert probe green; FAZ B-spawn-fail shipped 2026-09-11 — rejected headless runs persist an honest `[run failed: ...]` marker row to the transcript (REQ-070 F5-proof; new pure `formatClaudeRunFailed`, engine markers pass through verbatim, abort framing untouched, warn-only persist in ws.ts catch), 140-assert probe green; FAZ B-run-error shipped 2026-09-11 — error_during_execution result persists honest [run stopped: error_during_execution] marker (same vocabulary as FAZ A max_turns/max_budget; unknown/missing subtype normalization + cost/session carry probed), 148-assert probe green)
