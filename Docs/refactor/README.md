...
- [REQ-151](REQ-151-pencere-alta-tasma-clamp.md) — Windowed mode overflows at the bottom: the y clamp reserves only 40px for the title bar while x keeps the whole window inside — pending
- [REQ-152](REQ-152-pencere-focus-one-gelsin.md) — Clicking a floating window must raise it to the front (Windows-style z-order + focus ring, persisted) — pending