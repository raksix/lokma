...
import { ChevronDown, ChevronRight, Download, Layers, Puzzle, Search, Trash2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { ApiError, api, type Plugin } from '@/lib/api';
import {
  PLUGIN_CATEGORIES,
  categoryTone,
  filterPlugins,
...
/**
 * PluginsPane — kernel registry + hot toggle + add-from-URL (W6-23, Docs/23).
 ...
 * `DELETE /api/plugins/:id` (URL records only).
 * NOT ported: the concept's invented `downloads`/`stars` figures, the fake
 * marketplace rows with toast-only Install buttons, and the toast-only
 * Kernel button (the expander loads the REAL manifest now). There is no
 * remote marketplace yet — the tabs are Installed/Suspended over the real
 * local registry, and the footer says so.
 */
    ...
    <div className="h-full flex flex-col bg-white dark:bg-[#161618] rounded-lg overflow-hidden border border-line">
      <div className="h-7 flex items-center gap-1.5 px-3 border-b border-line bg-[#FDFCFB] dark:bg-[#1E1E21] shrink-0">
        ...
        <span className="ml-auto flex gap-1">
          ...
          <Button
            variant={tab === 'suspended' ? 'default' : 'ghost'}
            size="sm"
            className="h-5 px-2 text-[11px]"
      ...
      <div className="p-2 border-t border-line bg-muted/20 text-[11px] text-zinc-500 flex gap-1 flex-wrap">
        ...
        <span className="ml-auto hidden sm:inline">suspend/resume without restart — no remote marketplace yet</span>
      </div>
    </div>
  );
}
