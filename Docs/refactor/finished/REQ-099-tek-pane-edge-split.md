...
- **Touched:** `packages/lokma-web/web/src/components/panes/pane.tsx` (`handleDrop` tab-move branch: edge first, same-pane + center stays no-op) + live bundle `assets/index-B9l0bff3.js` (served == disk, BUNDLE-MATCH)
...
- **Note (2026-09-10):** secondary diagnosis already resolved in tree — `workspace.tsx` moveTab edge branch removes the tab from source (verified); only the `pane.tsx` guard changed.
