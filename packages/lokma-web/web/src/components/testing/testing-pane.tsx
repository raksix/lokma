        ...
        {loading ? (
          ...
          visible.map((r) => {
            ...
            return (
              ...
              >
                <button className="w-full px-2.5 py-2 text-left" onClick={() => void toggleExpand(r.id)}>
                  <span className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${DOT_CLASS[tone]}`} />
                    <span className="text-xs font-semibold truncate min-w-0 flex-1" title={r.plan}>{r.plan}</span>
                    <ChevronDown className={`w-3.5 h-3.5 text-zinc-400 transition shrink-0 ${open ? 'rotate-180' : ''}`} />
                  </span>
                  <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 pl-4">
                    <span className="text-[11px] px-1.5 py-0.5 rounded-full bg-muted border border-line font-mono shrink-0">
                      {r.id}
                    </span>
