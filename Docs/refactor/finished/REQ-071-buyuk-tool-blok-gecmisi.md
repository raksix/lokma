...
Follow-up (2026-09-08, recovery): (a) dead-upstream transcript note (commit b70d8ae) — stream failures now leave `[run failed: <reason>]` in the transcript instead of vanishing. (b) empty-first-turn guard in `agent-loop.ts` — a first turn with no text/tool-calls/questions throws an honest retryable error instead of showing "Response complete" with no response; later quiet turns still mean tool-work-done. (c) one-per-run quiet-turn nudge in `agent-loop.ts` — a later turn with no text/tool-calls/questions pushes a `<system>` continue nudge once per run instead of completing (tool-then-silence used to abandon real tasks: list_files ran, write_file never came); a second quiet turn still completes, no poke loops.
